  ...
  try {
    ...
    if (canAuthenticate) {
      isAuthenticated = await auth.authenticate(
        localizedReason: 'Scan your fingerprint to open Calculator',
        ...
        // Fallback if no sensor: Allow access (or handle explicitly)
        isAuthenticated = true; 
    }
  } catch (e) {
    // Handle error or allow fallback
    debugPrint("Auth Error: $e");
    isAuthenticated = true; // Fallback to allow usage if sensor fails
  }

  if (!isAuthenticated) {
     exit(0); // Exit if auth failed/cancelled
  }
